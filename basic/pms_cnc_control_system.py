...
class Run(Frame):
    L_dot_ref_cont = np.zeros([1,6])
    L_ref_cont = np.zeros([1,6])
    L_dot_cont = np.zeros([1,6])
    L_cont = np.zeros([1,6])
    time_cont = np.zeros(1)
...
#        self.set_ax()        
    ...
    def draw(self):
        self.L_dot_ref_cont = np.concatenate((self.L_dot_ref_cont, self.L_dot_ref))
